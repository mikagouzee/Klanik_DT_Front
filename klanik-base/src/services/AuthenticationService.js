...
import serv from "./KlanikService";
...
export default {
  ...
  async connect(myheaders) {
    ...
    return await client()
      .post("/connect/userinfo", { data: "" }, { headers: myheaders })
      .then(handleResponse)
      .then(res => {
        console.log("Connected!");
        serv.getKonsultantById(res.sub).then(r => {
          if (r.id) {
            store.dispatch("SetIsKonsultant", true);
          }
        });
  ...
  async askNewPassword(email) {
    ...
    return await client()
      .get(`api/Account/ForgotPassword?email=${email}`)
      .then(
        res => {
          console.log("Ok : ", res);
        },
  ...
  async resetPassword(resetModel) {
    return await client()
      .post("/api/Account/SaveNewPassword", resetModel)
      .then(res => {
        return res;
      });
  },
  ...
  async getToken(loginModel) {
    ...
    return client()
      .post("/connect/token", this.setFormData(loginModel), {
        headers: headers
      })
      .then(handleResponse)
      .then(resp => {
